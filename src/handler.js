...
const addBookHandler = (request, h) => {
  ...
  const isSuccess = buku.filter((buku) => buku.id === id).length > 0;
...
const getAllBooksHandler = () => {
  let books  = buku.map(({ id, name, publisher }) => ({id, name, publisher}));
  return{
    status: 'success',
    data: {
      books,
...
const getBookByIdHandler = (request, h) => {
  ...
  const book = buku.filter((n) => n.id === id)[0];
